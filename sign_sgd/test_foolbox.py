...
def attack(algorithm, dataset, targeted, norm='l2', num=50, stopping_criteria=None,
    ...
    for i, (xi,yi) in enumerate(test_loader):
        ...
        if targeted:
            ...
            adv = attack(image,label,iterations=6000,verbose=False, unpack=False, log_every_n_steps=100, starting_point=image_t)
        else:
            attack = foolbox.attacks.BoundaryAttack(fmodel)
            adv = attack(image,label,iterations=6000,verbose=False, unpack=False, log_every_n_steps=100)
        ...
        print("adversarial Example Found Successfully: distortion {} target {} queries {}".format(dis, np.argmax(fmodel.predictions(adv.image)), adv._total_prediction_calls))    

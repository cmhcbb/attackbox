...
class OPT_attack_polar_lsq(object):
    ...
    def attack_untargeted(self, x0, y0, alpha = 0.2, beta = 0.001, iterations = 1000):
        ...
        for i in range(iterations):
            gradient, grad_queries = self.eval_grad_least_squares(model, x0, y0, xg, initial_lbd = gg, tol=beta/500)
            gradient1, queries = self.eval_grad(model, x0, y0, xg, initial_lbd = gg, tol=beta/500)
            print("Numerical - Approx gradient cosine distance: {0}, Distance moved: {1}".format( 
                  scipy.spatial.distance.cosine(gradient.flatten(), gradient1.flatten()),
                  learning_rate*np.linalg.norm(gradient)))
    ...
    def eval_grad_least_squares(self, model, x0, y0, theta, initial_lbd, tol=1e-5,  h=0.01, percent_queries=0.5):
        fx = initial_lbd # evaluate function value at original point
        x = theta
        D = x.flatten().shape[0]
        ...
        for i in range(num_dirs):
            ...
            u /= np.linalg.norm(u)
            temp_x = x + h*u
            theta_ct = self.polar_to_ct(temp_x).reshape(*x0.shape)
            f1x, count = self.fine_grained_binary_search_local(model, x0, y0, theta_ct, initial_lbd = initial_lbd, tol=h/500)
